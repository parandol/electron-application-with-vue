...
class TCPSocketServer extends events.EventEmitter {
    ...
    startup() {
        ...
        server.on('listening', () => {
            this.emit("log", "Server listening");
        ...
        server.on('connection', (socket) => {
            ...
            socket.on('error', (err) => {
                this.emit("log", "Socket Error: " + JSON.stringify(err));
            ...
            socket.on('timeout', () => {
                this.emit("log", "Socket Timed out");
        ...
        server.on('close', () => {
            this.emit("shutdown");
            this.emit("log", "The server has been shut down successfully.");
            console.log('The server has been shut down successfully.');
        });
    ...
    write(socket, data) {
        ...
        if (!success){
            this.emit("log", "Client Send Fail");
